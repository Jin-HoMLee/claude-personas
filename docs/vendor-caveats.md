...
- Injection cap + scope (claude-personas#52, 2026-07-06): the SessionStart payload adopts Claude Code's own native role-index size guard (~200 lines / ~25 KB, whichever first; overridable via `PERSONAS_INJECT_BYTE_CAP` / `PERSONAS_INJECT_LINE_CAP`) rather than an invented figure, and injects the **role index only** - matching CC, which auto-injects the role `MEMORY.md` but not `shared/`. Shared is loaded on demand via the `load-persona-memory` skill, with a one-line pointer in the payload. This replaced the earlier 9000-char role+shared bound, which truncated every mature instance (cerebrum, all splice roles) and dropped shared entirely. (2026-07-06)
...
- Re-approval is keyed to `.codex/hooks.json` CONTENT: editing the inject script the hook points at does NOT re-trip the `/hooks` approval (verified live - the hook fired unchanged after the script was modified). Treat the memory repo's hook-script dir (`.agents/hooks/lib/`, formerly `scripts/`) as part of the trusted surface. (2026-07-04)
...
| 2 | Codex `additionalContext` ceiling | NO CEILING OBSERVED up to 17k chars (~4.3k tokens) on codex-cli 0.142.5: with the script's own cap raised to 60000, a 240-line index passed through intact (tail canary visible); the previously observed ~2.4k-token cut does NOT reproduce | 2026-07-04 | the stale ~2.4k figure is retired; the `[TRUNCATED ...]` trailer remains the safety net. (The 9000-char self-bound this row kept was later revised by #52 (2026-07-06): the cap now adopts CC's native ~200-line/~25 KB guard and the scope is role-index-only - see the Codex caveat above.) |
